  ...
  () => {
    class D3Canvas {
      ...
      add1DXAxis(xScale, y_px, fmt = () => "", ticks = []) {
        ...
        const xax = d3.axisBottom(xScale).tickValues(ticks).tickFormat(fmt);
    ...
    const cnv = new D3Canvas("#plotulps0", 0.9, 0.2, 600, {
      top: 30,
      right: 30,
      bottom: 35,
      left: 30,
    ...
    const xScale = d3.scaleLinear().domain([-1, 4]).range([0, cnv.w_px]);
    ...
    cnv.svg
      ...
      .attr(
        "transform",
        "translate(" + xScale(1.5) + "," + (yax_px - 50) + ")",
      );
  },
  false,
);
