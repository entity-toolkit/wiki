document.addEventListener(
  ...
  () => {
    ...
    const xlim = [0, 71];
    const ymax =
      (0.5 * (xlim[1] - xlim[0]) * (height - margins[1] - margins[3])) /
      (width - panel_w - margins[0] - margins[2]);
    ...
    const XY2PX = (x, y) => {
      var px =
        panel_w +
        margins[0] +
        ((width - panel_w - margins[0] - margins[2]) * (x - xlim[0])) /
          (xlim[1] - xlim[0]);
      var py =
        margins[1] +
        ((height - margins[1] - margins[3]) * (y - ylim[0])) /
          (ylim[1] - ylim[0]);
      return [px, height - py];
    };
    const S2PX = (s) => {
      return (
        (s * (width - panel_w - margins[0] - margins[2])) / (xlim[1] - xlim[0])
      );
    };
    ...
    const sketch = (p) => {
      ...
      p.setup = function () {
        ...
        savePNGbutton.parent(parent_id);
        ...
        const slider_r0_tr = p
          .createElement("tr")
          .parent(panel2)
          .style("display", "block");
        p.createElement("th", "r0:").parent(slider_r0_tr);
        input_r0 = p
          .createInput("0.0", "number")
        ...
        slider_h = p
          .createSlider(-0.5, 0.99, 0.4, 0.01)
          .parent(p.createElement("td").parent(slider_h_tr))
          .input(() => {
      ...
      const frame = (
        ...
        for (var j = 0; j <= nx2; j++) {
          ...
          var theta =
            x2 +
            (2.0 * h * x2 * (p.PI - 2.0 * x2) * (p.PI - x2)) / (p.PI * p.PI);
          var [x1, y1] = XY2PX(rmin * Math.sin(theta), rmin * Math.cos(theta));
          var [x2, y2] = XY2PX(rmax * Math.sin(theta), rmax * Math.cos(theta));
        ...
        p.text(
          "r0/r_min: " + String(parseFloat(String(r0)).toFixed(2)),
          (2 * width) / 3,
          20,
        );
