  ...
  const sketch = (p) => {
    ...
    p.setup = function () {
      const panel1 = p.createElement("tbody")
        .parent(p.createElement("table").parent(parent_id).style('display', 'inline-block').style('margin-right', '25px'));
      const panel2 = p.createElement("tbody")
        .parent(p.createElement("table").parent(parent_id).style('display', 'inline-block'));
