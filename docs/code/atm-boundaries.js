document.addEventListener("DOMContentLoaded", () => {
  ...
  const xticks = (s) => d3.axisBottom(s)
    .tickSizeOuter(0)
    .tickValues([0.05, 0.2])
    .tickFormat((v) => '');

  const addXAxes = (y0, scale, axgen) => svg
    .append("g")
    .attr('class', 'axes')
    .attr("transform", "translate(0," + y0 + ")")
    .call(axgen(scale));

  const addArrowhead = (ax, scale) => ax.append('path')
  ...
  svg.append("path")
    ...
    .attr('class', 'line');
  svg.append("path")
    .attr("d", curlyBrace(xscale(0.5), y0 - dy, xscale(0.2), y0 - dy, 20, 0.5))
    .attr('class', 'line');
  svg.append("path")
    .attr("d", curlyBrace(xscale(0.05), y0 + dy, xscale(0.2), y0 + dy, 20, 0.5))
    .attr('class', 'line');
  ...
  svg
    ...
    .attr("transform", "translate(" + xscale(0.1) + "," + (y0 - 5 * dy) + ")");
  ...
  svg
    ...
    .style("text-anchor", "middle")
    .attr("transform", "translate(" + xscale(0.35) + "," + (y0 - 5 * dy) + ")");
  ...
  svg
    ...
    .html("buffer zone")
    .style("text-anchor", "middle")
    .attr("transform", "translate(" + xscale(0.125) + "," + (y0 + 7 * dy) + ")");
  ...
  const pts = [[xscale(0.05), y0 - 7 * dy], [xscale(0.05), y0 + 7 * dy]];
  svg.append("path")
    .attr("d", d3.line()(pts))
